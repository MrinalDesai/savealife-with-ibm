...
I built SaveAlife solo over approximately 12 hours of focused work, using IBM Bob as my primary coding partner. **Bob wrote roughly 95% of the production code.** My role was to design the architecture, craft the prompts, integrate watsonx.ai, refine the medical guardrails, and iterate on the UI.

**Total Bob usage: approximately 8 Bobcoins (20% of the 40-coin hackathon budget) across 8 distinct tasks.**
...
| 2 | Data adapters | `HospitalsCsvAdapter`, `PharmaciesJsonAdapter`, `BloodBanksTextAdapter`, plus the `watsonx_client.py` wrapper around the `ibm-watsonx-ai` SDK | ~0.6 coins |
...
| 5 | Streamlit UI | Two-view app (Bystander + ER Trauma Bay), session state for an ER queue, sample reports, dark theme, pydeck map with labeled pins | included in pipeline task |
...
| 7 | **QR Code handoff** | New `qr/incident_qr.py` module that builds a compact incident summary and renders it as a scannable QR code; integrated into both the bystander view and ER trauma bay alert cards | ~1.0 coin |
| 8 | **Local Speech-to-Text** | New `voice/speech_input.py` module using `faster-whisper` (tiny model, fully offline) plus `streamlit-mic-recorder` UI integration; bystander can speak their report instead of typing | ~2.0 coins |
...
**Adherence to constraints.** When I told Bob *"don't modify the existing pipeline file"* or *"don't run any code, just write the files,"* it followed those instructions reliably. Each task ended with the right deliverables and no unsolicited extras. This was particularly important for the late-stage QR and speech tasks, where I needed Bob to add features in *isolated new folders* (`qr/`, `voice/`) so I could fall back if anything broke.
...
**Library-aware integration.** For the QR and speech tasks, Bob picked the right libraries (`qrcode[pil]`, `faster-whisper`, `streamlit-mic-recorder`), used them correctly, handled error cases, and produced clean public APIs (e.g., `transcribe_audio_bytes(bytes) -> str`) that integrated into the existing UI in just a few lines.
...
- **Explicit "do not modify" lists.** Telling Bob what *not* to touch was as important as telling it what to build. This was crucial for the late-stage QR and speech features, where breaking the working pipeline would have been catastrophic.
...
- **Isolated additions.** New features (QR, speech) lived in their own folders (`qr/`, `voice/`) so I could rip them out cleanly if they failed.

## Compliance

All watsonx.ai inference was performed against the hackathon-provisioned IBM Cloud account (account ID `2998220 - watsonx`), using the `ibm/granite-4-h-small` model. Total cloud spend was approximately $0.005 of the $80 credit allocation. The local speech-to-text feature uses `faster-whisper` running entirely on CPU — no IBM Watson STT API calls were made, which means the system functions even when the bystander has no network connectivity.

## Conclusion

Bob is best used as a **fast, literal, well-instructed collaborator** — not as an oracle. The 95/5 split between Bob-generated code and human-written prompts/refinements feels about right for a hackathon. Bob compressed two days of work into about 12 hours, and the modular structure (each feature in its own folder) meant late-stage additions like QR and voice input could be added without risking the working pipeline.